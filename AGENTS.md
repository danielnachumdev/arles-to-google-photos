...
- **Backend runtime:** `[project].dependencies` — `gp_wrapper>=0.9.11`, `beautifulsoup4`, `tqdm`, `fastapi`, `uvicorn`, `python-multipart`, `piexif>=1.1.3`, `google-cloud-storage>=2.14.0`, `sqlalchemy>=2.0.25`, `psycopg[binary]`, `alembic`
...
- **gp_wrapper 0.9.11+:** portable `FileTimeService` (no Win32 ctypes). Non-mp4 video transcode runs with or without `pbar` and copies source access/modification onto the local `.mp4`. `Dictable` serializes Enums (e.g. `PositionType`) so `AlbumPosition.to_dict()` is JSON-safe. `uv.lock` pins `gp-wrapper==0.9.11`.
