...
class AlbumPublisher:
    ...
    def publish(
        ...
        total = len(to_upload)
        self._stamper.stamp_upload(root, to_upload)
        if sink is not None:
        ...
        # Upload + batchCreate per batch so an interrupt mid-album does not leave
        # an empty Photos album (tokens alone are not attached until batchCreate).
